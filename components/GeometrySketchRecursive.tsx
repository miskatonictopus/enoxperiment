...
interface GeometrySketchRecursiveProps {
  paletteName?: string;
...
export default function GeometrySketchRecursive({ paletteName, captureRef }: GeometrySketchRecursiveProps) {
...
void main() {
  ...
  } else if (uGradientType == 5.0) {
    mixValue = 1.0 - (abs(st.x) + abs(st.y));
  } else if (uGradientType == 6.0) {
    float r = length(st);
    float a = atan(st.y, st.x);
    mixValue = 1.0 - mod(r + a / TWO_PI, 1.0);
  } else if (uGradientType == 7.0) {
    float a = atan(st.y, st.x);
    float r = length(st);
    float star = 0.5 + 0.5 * cos(5.0 * a);
    mixValue = 1.0 - (r / max(star, 0.0001));
  } else if (uGradientType == 8.0) {
    float a = atan(st.y, st.x);
    float segments = 8.0;
  ...
  if (uSpeed > 0.0) {
    ...
    } else if (uAnimationType == 3.0) {
      mixValue += sin(st.y * 10.0 + t * 3.0) * 0.1;
    } else if (uAnimationType == 4.0) {
...
`;
        ...
        class LightingGradientShader {
          ...
          setColors(colors: string[]) {
            this.config.colors = colors.map(hexToRgbUnit);
          }
          ...
          setDiffuseStrength(strength: number) {
          ...
          apply() {
            ...
            this.shader.setUniform('uColorCount', Math.min(16, this.config.colors.length));
            ...
            for (let i = 0; i < 16; i++) {
              const c = this.config.colors[i] ?? [0, 0, 0];
              this.shader.setUniform(`uColor${i}`, c);
            }
          }
        }
        ...
        class Element {
          ...
          angleYAccel: number;
          angleZAccel: number;
          shaderPass: LightingGradientShader;
          ...
          constructor(props: any = {}) {
            ...
            this.shaderPass = new LightingGradientShader(shuffleArray(palette));
            this.shaderPass.setGradientType(0);
            this.shaderPass.setAnimationType(5);
            this.shaderPass.setAngle(p.HALF_PI);
            this.shaderPass.setAlpha(1);
            this.shaderPass.setAmbientStrength(1);
            this.shaderPass.setDiffuseStrength(0);
          }
          ...
          run = () => {
            ...
            this.shaderPass.apply();
            p.cylinder(this.radius, this.h, 64, 64, true, true);
            p.resetShader();
            p.pop();
          };
        }
        ...
        class Motif {
          ...
          constructor(props: any = {}) {
            ...
            this.elements = [];
            this.maxLevel = 8;
            this.recursionCircle(0, 0, 1000, this.maxLevel);
          }

          recursionCircle = (x: number, y: number, radius: number, level: number) => {
            if (level < 1) return;
            this.elements.push(
              new Element({
                ...
                radius,
                h: radius * 10,
                angleY: p.random(0, p.TWO_PI),
              })
            );
            if (level > 1) {
              this.recursionCircle(x - radius / 2, y, radius / 2, level - 1);
              this.recursionCircle(x + radius / 2, y, radius / 2, level - 1);
            }
          };
          ...
          run = () => {
            ...
            p.rotateY(this.angleY);
            p.rotateZ(this.angleZ);
            const sorted = [...this.elements].sort((a, b) => a.id - b.id);
        ...
        const init = () => {
          ...
          motif = new Motif({ originX: 0, originY: 0, originZ: 0 });
          initCamera();
        };
  ...
  return <div ref={mountRef} className="w-full h-full bg-[#f5f5f5]" style={{ width: '100vw', height: '100vh' }} />;
}
